...
def run_calibration():
    ...
    # Initialisation des variables à None pour le bloc finally
    camera = None
    rtde_c = None
    ...
    try:
        ...
        try:
            ...
            print(f"❌ Erreur lors de la création des paramètres de capture : {e}")
            return

        # --- DÉFINITION DES POSES DE CALIBRATION ---
        calibration_poses = [
        ...
        # Vérification du nombre de poses valides minimum
        ...
        print(f"❌ Une erreur inattendue est survenue : {global_error}")

    finally:
        # Nettoyage de la caméra Zivid
        if camera is not None:
            del camera
            print("Camera connection closed.")
        
        # Libération du contrôle du robot
        if rtde_c is not None:
            try:
                rtde_c.stopScript()
                print("Robot RTDE script stopped.")
            except Exception as e:
                print(f"Erreur lors de la fermeture du RTDE : {e}")
